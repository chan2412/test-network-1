...
function createOrgs() {
  ...
  while :
    do
      if [ ! -f "organizations/fabric-ca/hospital1/tls-cert.pem" ]; then
        sleep 1
      elif [ ! -f "organizations/fabric-ca/hospital2/tls-cert.pem" ]; then
        sleep 2
      else
        break
      fi
  ...
  infoln "Generating CCP files for Hospital1 and Hospital2 and Laboratory"
...
# call the script to create the channel, join the peers of hospital1 and hospital2,
...
function networkDown() {
  # stop laboratory containers also in addition to hospital1 and hospital2, in case we were running sample to add laboratory
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf addOrg3/fabric-ca/laboratory/msp addOrg3/fabric-ca/laboratory/tls-cert.pem addOrg3/fabric-ca/laboratory/ca-cert.pem addOrg3/fabric-ca/laboratory/IssuerPublicKey addOrg3/fabric-ca/laboratory/IssuerRevocationPublicKey addorg3/fabric-ca/laboratory/fabric-ca-server.db'
...
# use this as the docker compose couch file for laboratory
COMPOSE_FILE_COUCH_ORG3=addOrg3/docker/docker-compose-couch-org3.yaml
# use this as the default docker-compose yaml definition for laboratory
