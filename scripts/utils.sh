...
function printHelp() {
  ...
  if [ "$USAGE" == "up" ]; then
    ...
    println "    -ccep <policy>  - (Optional) Chaincode endorsement policy using signature policy syntax. The default policy requires an endorsement from Hospital1 and Hospital2 and Laboratory"
