...
createAnchorPeerUpdate() {
  ...
  infoln "Generating anchor peer update transaction for Hospital${ORG} on channel $CHANNEL_NAME"
  ...
  if [ $ORG -eq 1 ]; then
    ...
    HOST="peer0.laboratory.example.com"
    PORT=10051
  else
    errorln "Hospital${ORG} unknown"
  fi
